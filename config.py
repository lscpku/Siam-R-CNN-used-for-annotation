...
# paths to datasets! changes these!
_C.DATA.IMAGENET_VID_ROOT = "/globalwork/data/ILSVRC_VID/ILSVRC/"
_C.DATA.GOT10K_ROOT = "/globalwork/data/GOT10k/"
_C.DATA.LASOT_ROOT = "/globalwork/data/LaSOTBenchmark/"
_C.DATA.YOUTUBE_VOS_ROOT = "/globalwork/data/youtube-vos/"
_C.DATA.DAVIS2017_ROOT = "/globalwork/data/DAVIS2017/"
_C.DATA.YOUTUBE_BB_ROOT = "/globalwork/data/youtube-bb/yt_bb_detection_train/"
_C.DATA.TRACKINGNET_ROOT = "/globalwork/data/TrackingNet/"
_C.HARD_MINING_DATA_PATH = "/globalwork/data/hard_example_mining_index/"

_C.DATA.IMAGENET_VID = True
_C.DATA.GOT10K = True
_C.DATA.LASOT = True
_C.DATA.YOUTUBE_VOS = True
_C.DATA.YOUTUBE_BB = False
_C.DATA.DAVIS2017 = False
_C.DATA.TRACKINGNET = False

# FIXME
_C.MODE_EXTRA_FEATURES = "half"
...
_C.MODE_FPN = True
_C.MODE_TRACK = True
_C.TRACK_VIDEO_ID = None

# new flags by us
_C.MODE_SHARED_CONV_REDUCE = False
_C.USE_PRECOMPUTED_REF_FEATURES = False
_C.EXTRACT_GT_FEATURES = False
_C.MODE_THIRD_STAGE = False
_C.EXTEND_PROPOSALS_BY_ACTIVE_TRACKLETS = True

# hard mining stuff
_C.MODE_HARD_MINING = False
_C.MODE_IF_HARD_MINING_THEN_ALSO_POSITIVES = True
_C.MODE_HARD_NEGATIVES_ONLY_CROSSOVER = False
_C.MODE_HARD_NEGATIVES_ONLY_CROSSOVER_YOUTUBEVOS = False
_C.USE_REGRESSION_LOSS_ON_HARD_POSITIVES = False
_C.HARD_NEGATIVE_LOSS_SCALING_FACTOR = 0.12
_C.HARD_POSITIVE_LOSS_SCALING_FACTOR = 0.1
_C.HARD_POSITIVE_BOX_LOSS_SCALING_FACTOR = 0.1
_C.N_HARD_NEGATIVES_TO_SAMPLE = 100
_C.N_HARD_POS_TO_SAMPLE = 30
_C.HARD_MINING_KNN = 10000
_C.HARD_MINING_KNN_LASOT = 50000

_C.FORWARD_VIDEO_RANGE_START = None
_C.FORWARD_VIDEO_RANGE_END = None

# might lead to minor slowdown, but gives useful information
_C.MEASURE_IOU_DURING_TRAINING = True
...
_C.DATA.DEBUG_VIS = False
_C.DATA.MULTITHREAD = True

_C.DATA.GRAYSCALE_AUGMENTATIONS = True
_C.DATA.MOTION_BLUR_AUGMENTATIONS = True
...
_C.TRAIN.MAX_NUM_EPOCHS = 1000000000000
...
#_C.TRAIN.LR_SCHEDULE = [1500000, 1580000, 1620000]
_C.TRAIN.LR_SCHEDULE = [250000, 280000, 300000] # for main training, afterwards we can do hard example training
...
# seems we need a lot of proposals for tracking with fixed RPN. Note that this makes it quite slow!
# to prevent OOM let's do 8k for validating during training
# for actual forwarding, we could do 15k
_C.RPN.TEST_PER_LEVEL_NMS_TOPK = 1000
_C.RPN.TEST_ALTERNATIVE_ANCHOR_SAMPLING = False
...
_C.FRCNN.USE_FOCAL_LOSS = False
_C.FRCNN.FG_LOSS_WEIGHTING_FACTOR = 3.0
_C.FRCNN.BOX_LOSS_WEIGHTING_FACTOR = 1.0
...
_C.TEST.RESULT_SCORE_THRESH = 0.005
_C.TEST.RESULT_SCORE_THRESH_VIS = 0.005   # only visualize confident results
...
def finalize_configs(is_training):
    ...
    assert _C.BACKBONE.FREEZE_AT in [0, 1, 2, 3, 4]
